...
bool runOnBasicBlock(BasicBlock &BB) {
    ...
    for (Instruction &Inst: BB) {
        ...
        if (auto *BinOp = dyn_cast<BinaryOperator>(&Inst)) {
            ...
            } else if(BinOp->getOpcode() == Instruction::Mul || BinOp->getOpcode() == Instruction::FMul || ((BinOp->getOpcode() == Instruction::SDiv || BinOp->getOpcode() == Instruction::FDiv || BinOp->getOpcode() == Instruction::UDiv) && nonConstantOperand == BinOp->getOperand(0))){
            ...
            if (BinOp->getOpcode() == Instruction::Mul) {
                ...
                if (ConstValue.isPowerOf2()) {//2,4,8,16,...
                    //shift amount = log2(constant)
                    ...
                    Transformed = true;
                    continue;
                } else if (ConstValue.countTrailingOnes() == ConstValue.popcount()) {//1, 3, 7, 15, ...
                    //total bits set == consecutive bits set to one from the LSB, es: 111
                    //shift amount: total bits set: es: x * 7 == (x << 3) - x
                    ...
                    Transformed = true;
                    continue;
                }else if(ConstValue[0] && ConstValue[ConstValue.getActiveBits()-1] && ConstValue.popcount() == 2){//3, 5, 9, 17, ...
                    //exactly two bits set: the MSB and the LSB. es: 101
            ...
            } else if ((BinOp->getOpcode() == Instruction::SDiv || BinOp->getOpcode() == Instruction::UDiv) && ConstOperand == Operand1) { //2,4,8,16
                //shift amount = log2(constant)
